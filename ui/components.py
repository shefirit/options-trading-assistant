...
def render_total_at_risk(risk: dict, capital: float, monthly_goal: float,
                         real_money: bool = True) -> None:
    """If every open trade went wrong at once - the number that only shows up
    by accident, usually in the week it matters.

    Sits above the open-trades table because it is about all of them together,
    and no per-trade row can ever say it.
    """
    if not risk["count"]:
        return
    total = float(risk["total"])
    defined = float(risk["defined"])
    pct = risk["pct_of_capital"]
    dpct = risk["defined_pct_of_capital"]

    # Coloured on the CAPPED part, which is the risk that can genuinely turn
    # up. Colouring the total would paint a book holding one cash secured put
    # permanently red, since its "max loss" assumes the stock reaches zero.
    tone = theme.INK
    if dpct is not None:
        tone = (theme.RED if dpct >= 0.20
                else theme.AMBER if dpct >= 0.10 else theme.INK)

    def _line(label: str, value: float, note: str, color: str,
              big: bool = False) -> str:
        size = "2.1rem" if big else "1.35rem"
        return (f"<div style='min-width:200px;flex:1 1 200px;'>"
                f"<div style='font-size:.78rem;font-weight:800;"
                f"color:{theme.SECONDARY};letter-spacing:.05em;'>{label}</div>"
                f"<div style='font-size:{size};font-weight:800;color:{color};"
                f"line-height:1.15;margin:2px 0;'>-{_dollars(value)}</div>"
                f"<div style='font-size:.9rem;font-weight:600;"
                f"color:{theme.SECONDARY};line-height:1.45;'>{note}</div></div>")

    total_note = (f"{pct * 100:.0f}% of your {_dollars(capital)}"
                  if pct is not None else "across every open trade")
    defined_note = (f"{dpct * 100:.0f}% of your capital"
                    if dpct is not None else "capped by a leg you own")

    st.markdown(
        f"<div style='border:1px solid {theme.BORDER_STRONG};border-radius:14px;"
        f"background:{theme.TILE};padding:16px 18px;margin:4px 0 10px;"
        f"display:flex;gap:26px;flex-wrap:wrap;'>"
        + _line("IF EVERY OPEN TRADE HIT ITS MAX LOSS", total, total_note,
                theme.INK, big=True)
        + _line("OF THAT, THE PART WITH A HARD CAP", defined, defined_note, tone)
        + "</div>",
        unsafe_allow_html=True)

    plural = "trade" if risk["count"] == 1 else "trades"
    theme.note(
        f"Every one of your **{risk['count']}** open {plural} losing the most it "
        f"possibly can, on the same day. Your SOP has no rule about this total; "
        f"it is here so you know it before a bad week tells you.")
    if risk["to_zero"] > 0:
        theme.note(
            f"The two numbers differ because **max loss does not mean the same "
            f"thing on every strategy**. On a spread, a condor or the PMCC it is "
            f"capped by a leg you own - that is the second number, and it can "
            f"really happen. The remaining **\\${risk['to_zero']:,.0f}** comes "
            f"from positions with no protective leg (cash secured puts, covered "
            f"calls), where max loss means the underlying reaching **\\$0**. "
            f"True arithmetic, not a scenario to plan around.")

    biggest = risk["biggest"]
    if biggest and biggest["loss"] > 0:
        theme.note(
            f"The single biggest position by worst case is **{biggest['underlying']} "
            f"{short_strategy(biggest['strategy'])}** at "
            f"**\\${biggest['loss']:,.0f}**.")
    if not real_money:
        theme.note("These are practice trades, so none of this money is real.")
    if risk["estimated"]:
        theme.note(f"{risk['estimated']} position(s) could not be priced just now, so "
                   "their share is the figure worked out at entry. Treat the total "
                   "as a floor rather than an exact number.")

    with st.expander("Break it down by trade"):
        st.dataframe(pd.DataFrame([{
            "Symbol": r["underlying"],
            "Strategy": short_strategy(r["strategy"]),
            "Most it can lose": r["loss"],
            "Capped?": "Yes - you own the protection" if r["defined"]
                       else "No - floor is zero",
        } for r in risk["rows"]]), width="stretch", hide_index=True,
            column_config={
                "Most it can lose": st.column_config.NumberColumn(
                    format="$%d",
                    help="On a spread or condor this was fixed the moment it "
                         "filled. On a cash secured put or covered call it "
                         "assumes the underlying goes to zero."),
                "Capped?": st.column_config.TextColumn(
                    help="Whether a leg you own puts a hard limit on the loss."),
            })
