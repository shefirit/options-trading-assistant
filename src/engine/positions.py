...
def worst_case_loss(position: Position,
                    underlying_price: Optional[float] = None) -> float:
    """The most this one position can still lose, as a positive number.

    On the credit shapes that is the max loss worked out at entry: a spread's
    risk is fixed the moment it is filled and nothing that happens later
    changes it.

    On the covered call models it is NOT. Their stored "max loss" is the cash
    laid out for shares she still owns, which is not a loss at all - it ignores
    the protective put that is the whole point of Model 1, and understates
    Model 3, where the short puts make the fall accelerate. For those the
    number comes from the real payoff, walked down to zero.
    """
    if position.shares_cost > 0:
        read = protection_read(position, underlying_price)
        if read is not None:
            return round(abs(min(read["worst_case"], 0.0)), 2)
    return round(max(position.max_loss, 0.0), 2)


def has_defined_risk(position: Position) -> bool:
    """True when the loss is capped by something she OWNS, not by hope.

    A credit spread and an iron condor buy a protective leg, and a PMCC buys
    the LEAPS - on all of those the worst case is a real, bounded number that
    can genuinely turn up.

    A cash secured put and the covered call models are different: their "worst
    case" is the underlying going to ZERO. That is the true floor and it is
    also not a thing that happens to SPY. Keeping the two apart is the whole
    point - added together they produce one alarming number that hides how much
    of the risk is real.
    """
    if position.shares_cost > 0:
        return False
    return any(leg.action == Action.BUY for leg in position.legs)


def total_at_risk(positions: list[Position],
                  prices: Optional[dict[str, float]] = None,
                  capital: float = 0.0) -> dict[str, Any]:
    """If every open trade hit its max loss at the same time.

    A number no beginner computes and every beginner needs: trades are checked
    one at a time, so the total only ever appears by accident, usually in the
    week it matters. It is deliberately the plain SUM, with no allowance for
    the fact that they are unlikely to all fail together - a worst case that
    quietly assumes things go only half wrong is not a worst case.

    The same total is also split, because on its own it reads as one kind of
    risk when it is really two:

      defined   the part capped by a leg she owns - a spread, a condor, the
                PMCC's long call. A number that can genuinely turn up.
      to_zero   the rest, from positions with no protective leg, where "max
                loss" means the underlying reaching zero.

    prices maps underlying -> current price, needed only by the covered call
    models. Positions missing one fall back to their stored max loss and are
    counted in `estimated`, so the UI can say the total is a floor.
    """
    prices = prices or {}
    rows: list[dict[str, Any]] = []
    estimated = 0
    for p in open_positions(positions):
        price = prices.get(p.underlying)
        if p.shares_cost > 0 and price is None:
            estimated += 1
        rows.append({
            "position": p,
            "underlying": p.underlying,
            "strategy": p.strategy_name,
            "loss": worst_case_loss(p, price),
            "defined": has_defined_risk(p),
        })
    rows.sort(key=lambda r: r["loss"], reverse=True)
    total = round(sum(r["loss"] for r in rows), 2)
    defined = round(sum(r["loss"] for r in rows if r["defined"]), 2)
    return {
        "total": total,
        "defined": defined,
        "to_zero": round(total - defined, 2),
        "rows": rows,
        "count": len(rows),
        "estimated": estimated,
        "pct_of_capital": (total / capital) if capital > 0 else None,
        "defined_pct_of_capital": (defined / capital) if capital > 0 else None,
